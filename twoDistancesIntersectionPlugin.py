    ...
    def addResultToLayer(self,interLayerPoint):
        if self.layer.isEditable():
            ...
            if self.layer.geometryType()==QgsWkbTypes.PointGeometry:
                geometry=QgsGeometry.fromPointXY(QgsPointXY(interLayerPoint))
                if self.featureFormEnabled():
                    #Service layer with fields attributes
                    f=QgsFeature(self.layer.fields(), self.layer.featureCount())
                    f.setGeometry(geometry)
                    if iface.openFeatureForm(self.layer,f):
                        self.layer.addFeature(f)
                else:
                    #Service layer without fields attributes
                    f=QgsFeature()
                    f.setGeometry(geometry)
                    self.layer.addFeature(f)
            ...
            if self.layer.geometryType()==QgsWkbTypes.LineGeometry:
                geometry1=QgsGeometry.fromPolylineXY([self.layerPointsXY[0],QgsPointXY(interLayerPoint)])
                if self.featureFormEnabled():
                    f1=QgsFeature(self.layer.fields(), self.layer.featureCount())
                    f1.setGeometry(geometry1)
                    if iface.openFeatureForm(self.layer,f1):
                        self.layer.addFeature(f1)
                else:
                    f1=QgsFeature()
                    f1.setGeometry(geometry1)
                    self.layer.addFeature(f1)
                
                geometry2=QgsGeometry.fromPolylineXY([self.layerPointsXY[1],QgsPointXY(interLayerPoint)])
                if self.featureFormEnabled():
                    f2=QgsFeature(self.layer.fields(), self.layer.featureCount())
                    f2.setGeometry(geometry2)
                    if iface.openFeatureForm(self.layer,f2):
                        self.layer.addFeature(f2)
                else:
                    f2=QgsFeature()
                    f2.setGeometry(geometry2)
                    self.layer.addFeature(f2)
            ...
            if self.layer.geometryType()==QgsWkbTypes.PolygonGeometry:
                geometry=QgsGeometry.fromPolygonXY([[self.layerPointsXY[0],
                                                     QgsPointXY(interLayerPoint),
