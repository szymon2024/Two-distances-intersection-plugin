...
#Float numbers
    ...
    #Calculate float spaces for x1,x2,y1,y2
    #for EPSx2,EPSy2
    spx1=nextafter(x1,inf)-x1
    spx2=nextafter(x2,inf)-x2
    spy1=nextafter(y1,inf)-y1
    spy2=nextafter(y2,inf)-y2
    ...
    EPSx2=sqrt(fsum([spx1*spx1,spx2*spx2])) #Range x2 meaning zero
    EPSy2=sqrt(fsum([spy1*spy1,spy2*spy2])) #Range y2 meaning zero
    if (x2==0 and y2==0) or (-EPSx2<x2<EPSx2 and -EPSy2<y2<EPSy2): #The circles coincide
        return (0,[])
    ...
    csq=c*c
    EPS=8*(nextafter(csq,inf)-csq)
    
    #The d0 and r1 comparison is the intersection criterion
    if csq>fsum([r1sq*dabsq,EPS]): #No intersection points
        return (0,[])
    else: #One intersection point
        if fabs(fsum([csq,-r1sq*dabsq]))<=EPS:
            #Restore circles position
            return (1,[fsum([x0,x1]),fsum([y0,y1])])
            ...
            ay=y0-a*m
            by=y0+a*m
            return(2,[fsum([ax,x1]),fsum([ay,y1]),fsum([bx,x1]),fsum([by,y1])])
