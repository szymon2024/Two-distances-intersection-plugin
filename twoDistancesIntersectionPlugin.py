    ...
    def lockDistanceChanged(self,locked):
        if locked:
            ...
            if self.cadDockWidget.constraintDistance().value()==0:
                iface.statusBarIface().showMessage('The distance can not be 0. '+
                    'Please enter a value greater than zero or press escape to get out of the panel.')
            else:
    ...
    def cadCanvasPressEvent(self,e):
        ...
        if e.button()==Qt.RightButton and self.layerPointXY!=None and self.distance==0:
            ...
            if self.distance==0: #Entered zero
                ...
                self.layerPointXY=None
                iface.statusBarIface().showMessage('The distance can not be 0. '+
                        'Try to start again by left click or press escape to cancel.')
    ...
    def cadCanvasPressEvent(self,e):
        ...
        if self.distanceCapture2!=None:
            ...
            if distance>0:
                ...
                if self.m==-1:
                    iface.messageBar().pushWarning("Warning","I can not do the calculations. Distance or position ranges exceeded.")
                    iface.messageBar().widgetRemoved.connect(self.widgetRemoved)
                    return False
...
#Decimal numbers
    ...
    spx1=nextafter(x1,inf)-x1
    spy1=nextafter(y1,inf)-y1
    spr1=nextafter(r1,inf)-r1
    sp1x2=nextafter(x2,inf)-x2
    sp1y2=nextafter(y2,inf)-y2
    sp2x2=x2-nextafter(x2,-inf)
    sp2y2=y2-nextafter(y2,-inf)
    ...
    if (x2==0 and y2==0) or (e2x2<x2<e1x2 and e2y2<y2<e1y2): #The circles coincide
        return (0,[])
    ...
    ea=2*e1x2
    eb=2*e1y2
    e1x2sq=E.pow(x2,2,e1x2)
    e1y2sq=E.pow(y2,2,e1y2)
    er1sq=E.pow(r1,2,er1)
    er2sq=E.pow(r2,2,er2)
    ec=E.add(E.add(E.add(e1x2sq,e1y2sq),er1sq),er2sq)
    ...
    if r1<EPS or r2<EPS: #Can not calculate
        return (-1,[])
